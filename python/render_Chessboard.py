...
def determine_lod_level(camera, lod_bounds):
    ...
    # Coc based check
    # to_bounds_center = (min_distance_to_bound(camera.position, lod_bounds) - camera.position)
    # to_bounds_center = np.array([to_bounds_center.x, to_bounds_center.y, to_bounds_center.z])
    # z = np.linalg.norm(to_bounds_center)
    ...
    # min_distance = min_distance_to_bound(camera.position, lod_bounds)
    # max_distance = max_distance_to_bound(camera.position, lod_bounds)
    # coc_limit = 0.5
    # H = f + 2.0 * f * a  / (2.0 * coc_limit)
    # d_near = s * H / (H + (s - f))
    # d_far  = s * H / (H - (s - f))
    # print(f'd_near: {d_near}, d_far: {d_far}')
...
def render(base_dir, config: RenderConfig):
    lod_names = [ 'GoldKnight', 'GoldKing', 'GoldQueen' ]
    ...
    for lod_name in lod_names:
        # lod_levels[lod_name] = 0
        # scene.set_lod_level(lod_name, 0)
    ...
    #     testbed.frame()
    #     if lod_levels['GoldKnight'] == 0:
    #         image = testbed.render_graph.get_output("ReservoirSplatting.color").to_numpy()[:,:,:3]
    #         if final_img is None:
    #             final_img = image
    #         else:
    #             final_img = final_img  + image
    #         num_images += 1
    #         # passes['ReSTIRPass'].reset()
    #     if i % 3 == 0:
    #         for lod_name in lod_names:
    #             lod_level = lod_levels[lod_name]
    #             new_lod_level = (lod_level + 1) % max_lod_level
    #             scene.set_lod_level(lod_name, new_lod_level)
    #             lod_levels[lod_name] = new_lod_level

    #         # if new_lod_level == 0:
    #         #     passes['ReSTIRPass'].reset()
    # pyexr.write(os.path.join(output_dir, f"gt_1.exr"), final_img / num_images)
...
def render_pt(base_dir, config: RenderConfig):
    """PT baseline at a single frame (frame 163), accumulating num_images spp.

    Matches render_gt's camera path (focus-pull advanced to frame 163, no LoD
    switching) so the spp comparison is apples-to-apples with gt.exr.
    """
    reso = config.resolution
    output_dir = os.path.join(base_dir, config.output_dir)
    os.makedirs(output_dir, exist_ok=True)
    ...
    # advance the focus-pull animation to frame 163 (matches render_gt)
    for i in tqdm.tqdm(range(163)):
        testbed.frame()
        camera.focalDistance = camera.focalDistance + animation.get(i)

    # freeze the camera and accumulate num_images spp at frame 163
    n_spp = config.num_images
    final_img = None
    ...
    pyexr.write(os.path.join(output_dir, f"frame163.exr"), final_img)

def render_gt(base_dir, config: RenderConfig):
    lod_names = [ f'EgyptianCat{lod_index}' for lod_index in range(5) ]
...
def pt_with_args(base_dir, args):
    config_file = args.config_file
    with open(config_file, 'r') as f:
        config = yaml.safe_load(f)
        render_config = RenderConfig(
        ...
        render_pt(base_dir, render_config)
...
if __name__ == "__main__":
    ...
    parser_pt = subparsers.add_parser('pt', help='Render PT baseline at frame 163 (num_images spp)')
    parser_pt.add_argument('--config_file', type=str, help='Path to the config file')
    parser_pt.set_defaults(func=pt_with_args)
